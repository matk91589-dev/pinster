...
async function saveUserToDB() {
    ...
    try {
        let { data: user, error: selectError } = await supabaseClient
            .from('users')
            .select('*')
            .eq('username', savedName === '-' ? CONFIG.APP.DEFAULT_USERNAME : savedName)
            .maybeSingle();
        
        if (selectError) throw selectError;
        
        const userData = {
            username: savedName === '-' ? CONFIG.APP.DEFAULT_USERNAME : savedName,
...
async function loadUserFromDB() {
    ...
    try {
        const { data: user, error } = await supabaseClient
            .from('users')
            .select('*')
            .eq('username', CONFIG.APP.DEFAULT_USERNAME)
            .maybeSingle();
